...
import com.kyungbae.app.mapper.UserMapper;
...
public class UserServiceImpl implements UserService{
    ...
    public Map<String, Object> registUser(UserDto user) {
        ...
        } catch (Exception e) { // insert 쿼리 실행 시 예외가 발생 했을 경우
            e.printStackTrace();
            return Map.of("message", "서버 오류 발생");
        }

        return Map.of("message", "회원가입에 성공하였습니다.");
    }
    ...
    public Map<String, Object> loginUser(UserDto user) { // user: 로그인시 입력한 아이디, 비번
        ...
        if (selectedUser == null) { // 아이디가 틀렸을 경우
            return Map.of("message", "존재하는 아이디가 없습니다.");
        } else if (!selectedUser.getUserPwd().equals(user.getUserPwd())) { // 비밀번호가 틀렸을 경우
    ...
    public Map<String, Object> modifyUserProfile(String userId, MultipartFile file) {
        ...
        try {
            ...
            e.printStackTrace();
            return Map.of("message", "첨부파일 에러발생");
        }
        ...
        // 전체 성공
        return Map.of("message", "성공적으로 변경이 완료되었습니다.",
                "profileURL", newProfileURL); // 세션의 회원정보 갱싱을 위해
    }
