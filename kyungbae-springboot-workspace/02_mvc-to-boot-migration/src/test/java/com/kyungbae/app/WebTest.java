...
public class WebTest {
    ...
    public void 로그인_실패_테스트1() throws Exception {
        ...
        /*
        mockMvc.perform(MockMvcRequestBuilders.post("/user/signin.do")  // import static 을 통해 생략가능
                .param("userId", "noavailableid")
                .param("userPwd", "nopassword"))
                .andDo(MockMvcResultHandlers.print());  // import static을 통해 생략
        */
        MvcResult mvcResult = mockMvc.perform(post("/user/signin.do")             // 요청 url, 요청 전송방식
                                             .param("userId", "noavailableid") // 요청 파라미터
                                             .param("userPwd", "nopassword"))
                                     .andDo(print())                                          // 요청-응답 흐름 출력
                                     .andExpect(status().is3xxRedirection())                  // 응답코드가 302 인지 확인
                                     .andReturn();
        System.out.println("==== mvcResult로 응답 확인 ====");
    ...
    public void 아이디중복체크_테스트() throws Exception {
        MvcResult mvcResult = mockMvc.perform(get("/user/idcheck.do")
                        .param("checkId", "user01")) // 아이디 중복검사 (중복)
                .andDo(print())
