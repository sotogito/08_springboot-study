package com.sotogito.app.handler;
...
@Slf4j
@Component
public class ChatEchoHandler extends TextWebSocketHandler { //메시지 수신, 전송

    private List<WebSocketSession> sessionList = new ArrayList<>();

    /**
     * 1) afterConnectionEstablished : 클라이언트와 연결되었을 때 실행되는 메서드
    ...
    /**
     * 2) handleMessage : 해당 웹소켓 측으로 데이터(텍스트, 파일)가 전송되었을 경우 실행되는 메서드
     *
    ...
    public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
        for (WebSocketSession client : sessionList) {
            // 메시지유형(chat,entry,exit)|채팅메시지|발신자의아이디
            String msg = "chat|" + message.getPayload() + "|" + ((UserDto) session.getAttributes().get("loginUser")).getUserId();
            client.sendMessage(new TextMessage(msg)); //클라이언트 측의 onMessage 함수 실행
        }
    }

    /**
     * 3) afterConnectionClosed : 클라이언트와 연경이 끊겼을 때 실행되는 메서드
     *
